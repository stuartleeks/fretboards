    ...
    <div class="fretboard" :style="fretboardStyle">
        <div 
            ...
            <div 
                ...
                :key="fretIndex" 
                class="string-fret" 
                :class="{'single-fretmark': stringIndex ===0 && instrument.fretMarks[fretIndex] === 'single'}"
                :data-notex="getBubbleText(stringIndex, fretIndex)">
                <span 
                    v-if="getBubbleText(stringIndex, fretIndex)" 
                    class="bubble"
                    :style="{color: getBubbleTextColor(stringIndex, fretIndex), backgroundColor: getBubbleColor(stringIndex, fretIndex)}">
                        {{getBubbleText(stringIndex, fretIndex)}}
                </span>
                <div 
                    v-if="stringIndex === 0 && instrument.fretMarks[fretIndex] === 'double'" 
...
export default {
    props : {
        instrument: Object, /* TODO create ctor function for stronger typing: https://vuejs.org/v2/guide/components-props.html#Type-Checks */
    ...
    computed: {
        fretboardStyle() { 
            return {
                ...
                '--string-height': 10,
                '--string-top': 'calc(((var(--fretboard-height) - var(--string-height))/ var(--string-count)) / 2 - 2)',
            };
        }
    }
};
...
.string-fret::before, .string-fret:first-child::before {
